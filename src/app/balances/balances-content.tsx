...
export async function BalancesContent() {
  const [chains, aggregated, pending, claimable, recentActivity] =
    await Promise.all([
      getChainBalances(),
      getAggregatedAssets(),
      getPendingState(),
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Section A: Liquidity by Chain — hide section if no chains */}
      {hasChains && (
        <section>
      ...
      {/* Section B: Asset Aggregation — always show card, empty state if no data */}
      <section>
        <h2 className="mb-4 text-sm font-medium text-slate-500">
          Asset Aggregation
        </h2>
      ...
      {/* Section C: In-Flight & Claimable — always show cards, empty state when no data */}
      <section>
        <h2 className="mb-4 text-sm font-medium text-slate-500">
          In-Flight & Claimable
        </h2>
      ...
      {/* Section D: Recent activity (with filter) */}
