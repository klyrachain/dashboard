 ...
 * Receives order lifecycle events; broadcasts to SSE stream so connected clients refresh (no polling).
 * @see md/core-api-integration.md
 */

import { NextResponse } from "next/server";
import type { CoreAdminWebhookBody } from "@/types/core-api";
import { broadcast } from "@/lib/webhook-notifier";
...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.NODE_ENV === "development" && ORDER_LIFECYCLE_EVENTS.includes(payload.event)) {
      console.info("[admin webhook]", payload.event, payload.data?.transactionId);
    }
    broadcast();
    return NextResponse.json(
